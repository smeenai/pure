...
prompt_pure_setup() {
	...
	zstyle ':vcs_info:hg*+set-message:*' hooks hg-dotfiles-scm-info
	function +vi-hg-dotfiles-scm-info() {
		hook_com[branch]="$(_dotfiles_scm_info '[%s]')"
	}
	...
	[[ "$SSH_CONNECTION" != '' ]] && prompt_pure_username=' %F{154}%n@%m%f'
	...
	PROMPT+="%(?.%F{213}.%F{196})${PURE_PROMPT_SYMBOL:-❯}%f "
}
